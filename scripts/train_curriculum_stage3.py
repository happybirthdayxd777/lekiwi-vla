#!/usr/bin/env python3
"""
Phase 262: Resume Stage 3 Curriculum Training
==============================================
Stage 1 (5 epochs) + Stage 2 (10 epochs) completed in Phase 260.
Stage 3 (15 epochs) CRASHED at checkpoint save due to disk full.

This script resumes Stage 3 from stage2_r045.pt checkpoint,
continuing from where the previous run left off.

Checkpoint: results/phase260_curriculum_train/stage2_r045.pt
Data: phase227_extended_65ep.h5 (7589 frames, ALL goals)
Output: results/phase261_curriculum_train/
...
DEVICE = "cpu"
print(f"[Stage 3 Resume] Device: {DEVICE}")
...
def train_stage(policy, replay, epochs, lr, stage_name, batch_size, save_every=5):
...
def plot_curriculum_loss(s1_losses, s2_losses, s3_losses, output_dir):
    ...
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Loss')
    ax.set_title('Phase 261: Curriculum Training Loss (Stage 1+2 Complete, Stage 3 Resumed)')
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"{output_dir}/curriculum_loss.png", dpi=150)
    print(f"[Plot] Saved {output_dir}/curriculum_loss.png")


# ─── Load Stage 2 checkpoint (already trained) ─────────────────────────────────

CKPT_STAGE2 = "results/phase260_curriculum_train/stage2_r045.pt"
DATA_PATH   = "data/phase227_extended_65ep.h5"
OUTPUT_DIR  = "results/phase261_curriculum_train"
EPOCHS_3    = 15
BATCH_SIZE  = 32
LR          = 1e-4

print(f"\n{'='*60}")
print(f"Phase 262: Stage 3 Curriculum Resume")
print(f"{'='*60}")
print(f"  Loading Stage 2 checkpoint: {CKPT_STAGE2}")
print(f"  Data: {DATA_PATH}")
print(f"  Output: {OUTPUT_DIR}")
print(f"  Stage 3 epochs: {EPOCHS_3}")
print()

os.makedirs(OUTPUT_DIR, exist_ok=True)

print("[1] Loading Stage 2 policy checkpoint...")
...
print(f"  Loaded: stage={ckpt.get('stage')}, epoch={ckpt.get('epoch')}, loss={ckpt.get('loss'):.4f}")
...
print(f"\n[2] Stage 3: ALL goals ({EPOCHS_3} epochs)")
...
print(f"\n[3] Saving final + best checkpoints...")
...
print(f"  Saved: final_policy.pt (loss={s3_losses[-1]:.4f})")
...
print(f"  Saved: best_policy.pt (epoch={best_idx+1}, loss={s3_losses[best_idx]:.4f})")
...
print(f"\n[Stage 3 Complete] epochs={EPOCHS_3}, best_loss={s3_losses[best_idx]:.4f} (epoch {best_idx+1})")
