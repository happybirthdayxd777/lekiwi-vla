#!/usr/bin/env python3
"""Phase 66c: Detailed single episode trace"""
...
# Run single episode with detailed trace every 10 steps
print('Single episode trace (100 steps)...')
sim = LeKiWiSimURDF()
np.random.seed(0)
...
for step in range(100):
    ...
    if step % 10 == 0:
        print(f'  Step {step:3d}: pos=({sim.data.qpos[0]:.3f},{sim.data.qpos[1]:.3f}), '
              f'dist={dist:.3f}, reward={r:.3f}, cum={cumulative_reward:.3f}, '
              f'action_wheels=[{action_np[6]:.2f},{action_np[7]:.2f},{action_np[8]:.2f}]')

print(f'\nFinal: pos=({sim.data.qpos[0]:.3f},{sim.data.qpos[1]:.3f}), '
      f'dist={dist:.3f}, cum_reward={cumulative_reward:.3f}, reached={reached}')
