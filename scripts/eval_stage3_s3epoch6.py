#!/usr/bin/env python3
"""
Phase 265: Evaluate Stage 3 Curriculum Checkpoint (s3_epoch6.pt)
================================================================
Evaluation on LeKiWiSimURDF (correct physics, 200 steps per episode, 20 random goals).

Reports:
  - VLA (Stage 3) success rate vs P-controller baseline
  - Per-goal breakdown
  - Final distance distribution

Checkpoint: results/phase264_curriculum_train/s3_epoch6.pt
Architecture: CLIP ViT-B/32 + CrossAttention + Flow Matching (from train_curriculum_stage3.py)
"""
...
DEVICE = "cpu"
print(f"[Phase 265] Device: {DEVICE}")
...
class GoalConditionedPolicy(torch.nn.Module):
    """Same architecture as train_curriculum_stage3.py — must match exactly."""
...
# ─── Wheel Normalization (matching eval_cross_attention_urdf.py) ────────────
...
def evaluate_vla_policy(policy, n_episodes=20, max_steps=200, threshold=0.15):
    ...
    for ep in range(n_episodes):
        ...
        # Random goal in reachable range (same as training)
        ...
        for step in range(max_steps):
            ...
            # Apply only wheel actions (actions 6:9), arm stays at current pos
    ...
    print(f"\n  Stage3 VLA: {successes}/{n_episodes} = {100*sr:.0f}% SR | mean_fail_dist={mean_fail_dist:.3f} | mean_steps={mean_steps:.0f}")
    return sr, mean_fail_dist, steps_list


def evaluate_pcontroller_baseline(n_episodes=20, max_steps=200, threshold=0.15):
    """P-controller baseline (this should be ~100% with correct physics)."""
    ...
    for ep in range(n_episodes):
        ...
        for step in range(max_steps):
            ...
            if dist < threshold:
                successes += 1
                steps_list.append(step)
                print(f"  P-ctrl Ep {ep}: SUCCESS step={step} dist={dist:.3f}")
                break
            ...
            print(f"  P-ctrl Ep {ep}: FAIL final_dist={fail_dists[-1]:.3f}")
...
def main():
    ckpt_path = WORKDIR / "results/phase264_curriculum_train/s3_epoch6.pt"

    print("=" * 60)
    print("Phase 265: Evaluate Stage 3 Curriculum (s3_epoch6.pt)")
    print("=" * 60)
    ...
    if ckpt_path.exists():
        ...
        print(f"[Loaded] epoch={ckpt.get('epoch','?')} loss={ckpt.get('loss','?')} | missing={len(missing)} unexpected={len(unexpected)}")
    else:
    ...
    # Baseline first (should be ~100%)
    print("\n── P-controller Baseline ──")
    p_sr, p_fail, p_steps = evaluate_pcontroller_baseline(n_episodes=20, threshold=0.15)

    # Stage 3 VLA
    print("\n── Stage 3 VLA (s3_epoch6) ──")
    vla_sr, vla_fail, vla_steps = evaluate_vla_policy(policy, n_episodes=20, threshold=0.15)

    print("\n" + "=" * 60)
    print("Phase 265 Results Summary")
    print("=" * 60)
    print(f"  P-controller:  {100*p_sr:.0f}% SR | mean_fail={p_fail:.3f} | mean_steps={np.mean(p_steps) if p_steps else 0:.0f}")
    print(f"  Stage3 VLA:    {100*vla_sr:.0f}% SR | mean_fail={vla_fail:.3f} | mean_steps={np.mean(vla_steps) if vla_steps else 0:.0f}")
    print(f"  Gap:           {100*(p_sr - vla_sr):.0f}%-points")
    print("=" * 60)
    ...
    results = {
        ...
        "p_controller": {"sr": p_sr, "mean_fail_dist": p_fail, "mean_steps": float(np.mean(p_steps)) if p_steps else 0},
        "stage3_vla": {"sr": vla_sr, "mean_fail_dist": vla_fail, "mean_steps": float(np.mean(vla_steps)) if vla_steps else 0},
    }
    out_path = WORKDIR / "results/phase265_eval.json"
