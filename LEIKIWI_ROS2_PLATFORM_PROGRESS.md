...
## Phase 124 (2026-04-17 03:30 UTC) — Contact Jacobian Closed-Loop: 100% Goal Reach

### Phase: Phase 124

### ✅ 本次心跳完成事項

**Phase 124: 接觸 Jacobian 閉環驗證 — 突破性 100% SR**

**核心驗證結果（Phase 123 的延續）：**

接觸 Jacobian 的 `twist_to_contact_wheel_speeds(J_c^+ @ [vx*10, vy*10])` 端到端測試：

| 測試 | cmd_vel | 結果 | 移動方向 |
|------|---------|------|---------|
| Pure +X | (0.3, 0.0) | +0.572m, +0.237m | ✓ 正確 |
| Pure +Y | (0.0, 0.3) | -0.018m, +0.355m | ✓ 正確 |
| Diagonal | (0.2, 0.2) | +0.529m, +0.337m | ✓ 正確 |

**閉環 Goal-Tracking 測試（4 goals，200 steps/goal，P-controller kP=1.5）：**

| Goal | 結果 | 步數 |
|------|------|------|
| (0.5, 0.0) | **REACHED** (dist=0.195m) | step 78 |
| (0.35, 0.35) | **REACHED** (dist=0.196m) | step 78 |
| (0.0, 0.5) | **REACHED** (dist=0.197m) | step 96 |
| (-0.3, 0.4) | **REACHED** (dist=0.195m) | step 102 |

**4/4 goals reached = 100% SR**

vs 之前最佳啟發式控制器：
- ContYaw: 40-53% SR（Phase 121）
- Rotate+Forward: 20% SR（Phase 120）
- GridSearchController: ~24% positive reward

**突破原因：**

1. J_c^+ 直接預測：world-frame (vx, vy) → 所需車輪速度
2. 以前的方法（ContYaw）使用 base-frame M7-forward，需要旋轉 + 前向切換
3. 接觸 Jacobian 方法：連續、平滑的 world-frame 速度命令

### 🔍 架構現況

| Component | Status |
|-----------|--------|
| bridge_node.py | ✅ 完整（Phase 123 更新了 `twist_to_wheel_speeds`）|
| twist_to_contact_wheel_speeds() | ✅ 驗證正確（Phase 124）|
| k_omni physics | ✅ URDF sim 的主要 loco 引擎 |
| CameraAdapter | ✅ 20 Hz 後台執行緒 |
| CTF 安全整合 | ✅ 完整 |
| **閉環 goal-tracking** | ✅ **100% SR（Phase 124 突破）** |

### 🧭 下一步（下次心跳）

**Phase 125: 數據收集 + 訓練**

1. 使用 `ClosedLoopController` + 接觸 Jacobian 收集 100+ episodes
2. 訓練新 policy：state_dim=9，CLIP-FM
3. 或：Bridge 端到端整合測試（真實 ROS2 環境）

### 🚫 阻礙

- **無重大阻礙** — Phase 124 完成突破
- **wz (yaw) 尚未整合到 J_c** — 目前只用 (vx, vy)

### 📊 實驗記錄

| Phase | 內容 | 結果 |
|-------|------|------|
| p121 | ContYaw controller | SR=40-53% (best heuristic) |
| p123 | Contact Jacobian calibrated | J_c^+ 替代 _omni_kinematics |
| **p124** | **Jacobian closed-loop validation** | **100% SR (4/4 goals)** |
...
## [Phase 130 - 2026-04-17 09:30 UTC] — k_omni=15.0 is the ONLY locomotion source; Contact Physics DOES NOT WORK on URDF sim

### ✅ 已完成

**Phase 130 findings — CRITICAL ARCHITECTURE ISSUE discovered:**

**Test 1: Flow matching inference step count**
- 4-step: raw wheel max |2.49|, tanh-bounded max |0.500|
- 8-step: raw wheel max |4.45|, tanh-bounded max |0.500|  
- 16-step: raw wheel max |3.41|, tanh-bounded max |0.500|
- **Conclusion: tanh denorm bounds ALL step counts to [-0.5, 0.5] wheel speed — outputs are bounded**
- But: more steps = more variance in raw outputs (4-step: |2.49| vs 8-step: |4.45|)

**Test 2: P-controller SR WITH k_omni (normal) — 4/4 (100%) in 54-96 steps**
**Test 3: P-controller SR WITHOUT k_omni (k_omni=0) — 0/3 (0%)**
**Test 4: VLA (4-step, tanh bounded) SR WITHOUT k_omni — 0/3 (0%), weird positions**

**ROOT CAUSE — k_omni=15.0 is the SOLE locomotion source:**
```
Without k_omni: P-controller fails to move robot at all (pos drift ~0.1m over 200 steps)
With k_omni=15: P-controller achieves 4/4 SR in 54-96 steps
```

**Analysis of step() function mechanics:**
1. mj_step() runs with ctrl + air resistance + soft joint limits
2. k_omni forces applied AFTER mj_step using wheel_vel (actual physics result)
3. These forces ADD to existing xfrc_applied (cumulative each step)
4. k_omni=15 produces vx_kin*15 ≈ 0.0455*15 ≈ 0.68 N horizontal force
5. This horizontal force slides the base toward goal direction

**The contact physics (wheel-ground) in URDF sim is COMPLETELY NON-FUNCTIONAL for locomotion.** The URDF has 3 omni wheels with contact geoms, but:
- Contact Jacobian J_c produces non-zero wheel→base mapping
- Yet pure contact locomotion (k_omni=0) produces ~0 movement
- Only the artificial k_omni overlay (kinematic model) works

### 🔍 Architecture Issue Summary

| Component | Status | Notes |
|-----------|--------|-------|
| Contact physics (wheel-ground) | **BROKEN** | k_omni=0 → no locomotion |
| k_omni overlay (kinematic model) | **ACTIVE** | k_omni=15.0, sole locomotion source |
| P-controller + k_omni | **100% SR** | Data collector works |
| VLA (task_oriented_50ep) | **0% SR** | Trained on k_omni-contaminated data |
| normalize_action (tanh) | **BOUNDED** | Outputs ∈ [-0.5, 0.5] for wheels |
| Flow matching inference | **STABLE** | More steps = more variance, but tanh bounds |

### 📊 Experiment Results

| Test | k_omni | Controller | Result |
|------|--------|------------|--------|
| Phase 129 | 15.0 | P-controller | 100% SR |
| Phase 130 | 15.0 | P-controller | 4/4 (54-96 steps) |
| Phase 130 | 0.0 | P-controller | 0/3 FAIL (pos drift ~0.1m) |
| Phase 130 | 0.0 | VLA (4-step, tanh) | 0/3 FAIL (weird positions) |
| Phase 130 | 15.0 | VLA (8-step) | 0/4 FAIL (0/4 goals) |

### 🧭 下一步（下次心跳）

**PRIORITY 1: Fix root cause — contact physics broken, k_omni is a workaround**
1. Investigate WHY contact physics doesn't work in URDF sim
   - Check wheel geom contact parameters (friction, contype, conaffinity)
   - Verify wheel-ground contact is actually occurring (ncon > 0 during steps)
   - Test: does anything produce locomotion without k_omni at all?
2. Fix URDF contact parameters to make real wheel-ground contact work
3. Remove k_omni artificial overlay once contact physics is fixed

**PRIORITY 2: VLA trained on k_omni-contaminated data**
- VLA learns to rely on k_omni physics (which is artificial)
- Need to retrain VLA on data WITHOUT k_omni overlay
- OR: keep k_omni as the locomotion model (it's a valid kinematic model)

**PRIORITY 3: Architecture decision**
- Option A: Fix contact physics (realistic, physics-based)
- Option B: Accept k_omni as the locomotion model (kinematic, works well)
- Option B is acceptable since k_omni is calibrated against contact Jacobian

### 🚫 阻礙

1. **Contact physics completely non-functional** — wheel-ground contact produces zero base locomotion
2. **k_omni is a band-aid** — artificial kinematic overlay masking the broken contact model
3. **VLA trained on contaminated data** — 100% k_omni SR data won't generalize to real contact physics

### 📋 Phase 131 Plan

1. Diagnose contact physics failure (add debug logging for ncon, contact forces)
2. Fix URDF wheel contact parameters (friction, restitution, contype/conaffinity)
3. Re-test pure contact locomotion
4. If contact physics can't be fixed, make k_omni the official locomotion model
5. Retrain VLA on clean (k_omni-only or contact-only) data

### Git

- Commit: Phase 130 — k_omni=15.0 is SOLE locomotion source; contact physics broken; P-controller 100% SR with k_omni, 0% without; VLA 0% SR with/without k_omni; need contact physics fix
...
- 8-step: raw wheel max |4.45|, tanh-bounded max |0.500|  
- 16-step: raw wheel max |3.41|, tanh-bounded max |0.500|
- **Conclusion: tanh denorm bounds ALL step counts to [-0.5, 0.5] wheel speed**
- More steps = more variance in raw outputs (not more stability)

**Test 2: P-controller SR WITH k_omni=15 — 4/4 (100%) in 54-96 steps**
**Test 3: P-controller SR WITHOUT k_omni (k_omni=0) — 0/3 (0%)**
**Test 4: VLA (4-step, tanh bounded) WITHOUT k_omni — 0/3 (0%), weird positions**

**ROOT CAUSE: k_omni=15.0 is the SOLE locomotion source.**
- Without k_omni: P-controller fails to move robot (0.1m drift over 200 steps)
- With k_omni=15: P-controller achieves 4/4 SR in 54-96 steps
- Contact physics (wheel-ground) in URDF sim is COMPLETELY NON-FUNCTIONAL

| Component | Status | Notes |
|-----------|--------|-------|
| Contact physics | BROKEN | k_omni=0 → no locomotion |
| k_omni overlay | ACTIVE | k_omni=15.0, sole locomotion |
| P-controller + k_omni | 100% SR | Data collector works |
| VLA (task_oriented_50ep) | 0% SR | Trained on k_omni data |
| normalize_action (tanh) | BOUNDED | Outputs [-0.5, 0.5] |

### 🧭 下一步（Phase 131）

**PRIORITY 1: Diagnose contact physics failure**
- Check wheel geom contact parameters
- Verify ncon > 0 during steps
- Test: does pure contact ever produce base motion?

**PRIORITY 2: Fix URDF contact parameters**
- Adjust friction, contype/conaffinity
- Re-test pure contact locomotion

**PRIORITY 3: Retrain VLA on clean data**
- Either without k_omni (if contact fixed)
- OR on k_omni-only data (if contact stays broken)

### 🚫 阻礙

1. Contact physics completely non-functional
2. k_omni is artificial band-aid
3. VLA trained on contaminated data
